            ...
            <form onSubmit={(event) => submit(event, false)} className="space-y-6">
                ...
                <section className="rounded-lg border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-950">
                        ...
                        <table className="min-w-full divide-y divide-slate-200 text-sm dark:divide-slate-800">
                            <thead className="bg-slate-50 dark:bg-slate-900">
                                <tr>
                                    ...
                                    <th className="px-3 py-3 text-left">Harga Jual</th>
                                        ...
                                        <tr key={index}>
                                            <td className="min-w-64 px-3 py-3">
                                                <Select value={item.katalog_id} onChange={(e) => setKatalog(index, e.target.value)}>
                                                    ...
                                                    {katalog.map((option) => <option key={option.id} value={option.id}>{option.label}</option>)}
                                                </Select>
                                            </td>
                                            ...
                                            <td className="w-36 px-3 py-3"><Input type="number" min="0" value={item.harga_satuan} onChange={(e) => updateItem(index, 'harga_satuan', e.target.value)} /></td>
                                            <td className="w-36 px-3 py-3"><Input value={item.hpp_satuan} readOnly /></td>
                                            <td className="whitespace-nowrap px-3 py-3 text-right">{money(total)}</td>
                                            <td className="whitespace-nowrap px-3 py-3 text-right">{money(profit)}</td>
                    ...
                    <div className="grid gap-3 border-t border-slate-200 p-4 text-sm dark:border-slate-800 sm:grid-cols-3">
                        <div className="rounded-md bg-slate-50 p-3 dark:bg-slate-900">Total Keseluruhan: <strong>{money(totals.total)}</strong></div>
                        <div className="rounded-md bg-slate-50 p-3 dark:bg-slate-900">Total HPP: <strong>{money(totals.totalHpp)}</strong></div>
                        <div className="rounded-md bg-slate-50 p-3 dark:bg-slate-900">Total Profit: <strong>{money(totals.profit)}</strong></div>
                    </div>
